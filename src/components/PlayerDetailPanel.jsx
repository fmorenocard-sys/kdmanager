...
const PlayerDetailPanel = ({ player, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm transition-all duration-300" onClick={onClose}>
            <div
                className="w-full max-w-md h-full bg-[var(--background)] border-s border-[var(--border-flat)] shadow-2xl transform transition-transform duration-300 ease-out overflow-y-auto"
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className="sticky top-0 z-20 bg-[var(--background)] border-b border-[var(--border-flat)] p-6 flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        <Avatar id={player.id} name={player.name} size="xl" showRing={true} />
                        <div>
...
const StatRow = ({ label, value, icon, color = "text-slate-400" }) => (
    <div className="flex justify-between items-center p-2 rounded-lg hover:bg-white/5 transition-colors">
        ...
        <span className="text-slate-200 font-mono font-medium tabular-nums">{new Intl.NumberFormat('en-US').format(value)}</span>
    </div>
);
