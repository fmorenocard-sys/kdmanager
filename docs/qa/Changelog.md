...
- **Le graphe d'évolution suit désormais la course, pas seulement le duel.** Il ne traçait que les deux camps de `hero_duel` : sur une course à trois alliés, la trajectoire du troisième était invisible. Il trace maintenant **une courbe par camp de la course** — union des camps du duel et de ceux marqués « nous » / « concurrent allié » — l'écart du duel restant en pointillés. Aucun changement de moteur ni rejeu de scan : `campDuel()` agrège sur `dkp_net`, exactement ce que porte déjà chaque entrée de `scan.camps`. Titre adapté (`kvk_race.evolution_race_title`, 10 locales) au-delà de deux camps. **Non-régression prouvée sur données réelles** : rejouée sur `soc4_race_2026` (6 scans), la nouvelle logique redonne les valeurs stockées à l'unité près — scan #6, East-Anglia 1 075 586 926 506 − Wessex 991 332 234 138 = 84 254 692 368, soit l'`ecart` enregistré.
- **Rôle corrigé sur Mimoso** : le camp Fire avait été marqué « concurrent allié » la veille pour que le formulaire auto-déduise le duel, alors que le Roi l'avait seulement désigné comme **adversaire** du duel — le badge affiché aux joueurs mentait. Remis à `adversaire`. L'union duel + concurrents garantit qu'il reste tracé dans le graphe malgré ce changement.
...
### Security
- **SEC-001 — CORRIGÉ sur les 4 instances.** Les classeurs bruts de 2997 (`KD 97 Bank Ledger.xlsx`, `KD 97 Deadweight.xlsx` nominatif, scans SoC, Top 300, 120 avatars) étaient servis **publiquement, sans authentification**, par le Hosting de 2997, 3341 et Arcelia — `public/data/` est copié tel quel dans `dist/` par Vite alors que rien ne le lit au runtime. `"**/data/**"` ajouté au `hosting.ignore` des quatre configs + redéploiement de chacune. Vérifié au type MIME et à la taille (le code HTTP ne prouve rien : la réécriture SPA `**` renvoie 200 pour tout chemin).
- **SEC-002 — CORRIGÉ sur les 4 instances.** Le bundle Cloud Functions emportait `serviceAccountKey.json` + `service-account.json` (2997), **les clés admin des autres clients** (`kd-41-manager.json`, `kd-2293-manager.json`) et `all_users.json` (**PII réelles**). Bloc `ignore` posé sur les configs clientes ; sur 2997 `service-account.json` est volontairement conservé (`runFullSync` en a besoin). Vérifié en téléchargeant les archives de sources réellement déployées (`gcf-v2-sources-*`), pas seulement la config.
- **2997 corrigé SANS livrer de code.** Son Hosting et ses Functions étaient tous deux sur `fc81c0a` (19/08), 15 commits derrière `main` : un redéploiement depuis `HEAD` aurait mis F-038, BUG-009, BUG-011 et 10 fichiers de locales en production. Le correctif a donc été construit depuis un **worktree sur `fc81c0a`**, build comparé au bundle en ligne (taille identique à l'octet ; 548 octets de divergence, tous des noms d'identifiants minifiés). Le footer de version affiche toujours `main · fc81c0a`.
- **SEC-003 ouvert** (voir `Issue_Backlog.md`) — trouvaille annexe de la vérification : chaque bundle Functions emporte les `functions/.env.<projet>` des **autres** instances. Sévérité faible (non-secrets par conception), non traité pour ne pas relancer un cycle de déploiement qui recrée les crons.
- **Rappel confirmé par les faits** : le redéploiement des Functions a **recréé `scheduledSync` sur 3341 et Arcelia** — la procédure obligatoire de l'Annexe A n'est pas théorique. Neutralisé par pause puis supprimé sur les deux. Le cron de 2997, lui, est légitime et reste actif.
...
### Fixed (suite — remontée Roi post-déploiement)
- **Bandeau KvK Config incohérent (BR-013)** : le panneau « Active Campaign — Currently Live » (vert, King-only) ignorait le statut `closed` et affichait SoC 4 comme active. Il est désormais sensible au statut : variante ambre « Campagne clôturée — inter-saison » (icône Archive, note « Aucune campagne active », clé `war.campaign_closed` ×15) ; redevient vert automatiquement à la sauvegarde de la prochaine campagne. Vérifié en dev (mock local du statut clôturé, retiré avant commit). Fix au passage : `prev` inutilisé dans `setSavedConfig` (erreur lint préexistante).
### Data
