...
| **BR-015** | **Per-instance module activation (F-023)** | Fixed modules (never toggleable): Dashboard, Profile, Admin, War Tracker, KvK Hub's Performance and Progression tabs. Optional modules (toggleable per white-label instance): Bank, Trophies, Deadweight, and the KvK Hub's Race/Course tab — gating operates at two granularities, page-level for the first three and tab-level for Race. Cascade rule: the Dashboard's embedded Bank block must follow the Bank module's toggle (disabled Bank ⇒ block hidden), even though the Dashboard itself is fixed; the Dashboard's power-evolution block is an empty-state concern, out of scope. Gating is presentation-only (no Firestore rules closure implied), mirroring BR-009/BR-011. Decided by the King 2026-07-27; see `Etude_Activation_Modules.md`. |
