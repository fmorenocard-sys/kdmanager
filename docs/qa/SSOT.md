...
| **BR-015** | **Per-instance module activation (F-023)** | Fixed modules (never toggleable): Dashboard, Profile, Admin, War Tracker, the **entire** KvK Hub (Performance, Progression, **and Race/Course tabs**). Optional modules (toggleable per white-label instance, **page-level only**): Bank, Trophies, Deadweight. Gating operates at a single granularity — page-level — since no optional module is a sub-tab. *Amended 2026-07-27 (same day, corrected arbitration)*: KvK Race was initially classified optional at tab-granularity; the King reclassified it **fixed** — "the KvK Race necessarily happens whenever there is a KvK, so it must be present." When no race scan is loaded on an instance, the Race tab shows an **empty state** (existing `EmptyState` component) rather than being hidden — an empty-state concern (UX hygiene), not a module-activation concern. Cascade rule (unchanged): the Dashboard's embedded Bank block must follow the Bank module's toggle (disabled Bank ⇒ block hidden), even though the Dashboard itself is fixed; the Dashboard's power-evolution block is a separate empty-state concern, out of scope. Gating is presentation-only (no Firestore rules closure implied), mirroring BR-009/BR-011. Decided by the King 2026-07-27; see `Etude_Activation_Modules.md`. |
| **BR-016** | **Multi-account model (proposed, E-007/F-025)** | A user may claim multiple governor accounts (`user_profiles/{uid}.accounts[]`), designating exactly one as `main` — never zero once at least one account is claimed, never two. Non-designated accounts are `filler` by construction (derived from the main designation, not a separate tag, and not derived from the campaign-scoped `static_data/kvk_filler` roster). The legacy single `governorId` field is kept as a mirror of the main account for backward compatibility with all existing readers. Claiming remains self-service (no officer validation), unchanged from the current single-account model. Proposed by the King 2026-07-27; see `Spec_Multicomptes_MainFiller.md` §4–§6. |
| **BR-017** | **Per-account war availability identity (proposed, E-007/F-026)** | `war_availabilities` document IDs extend from `${kvkId}_${uid}` to `${kvkId}_${uid}_${governorId}`, allowing one declaration per claimed account per campaign. Guest IDs (`${kvkId}_guest_${governorId}`) are unchanged. Pre-existing documents (legacy 2-segment ID) are read as the declaration of the user's `main` account — no batch rewrite; new declarations for any account, including main, use the 3-segment scheme going forward. Proposed by the King 2026-07-27; see `Spec_Multicomptes_MainFiller.md` §7.2. |
| **BR-018** | **Filler goal scale — dedicated points, never mixed (proposed, E-007/F-027)** | The filler goal formula uses a points scale where 1 T4 = 4 points and 1 T5 = 10 points: declared power (points) = 4×T4 + 10×T5; death-loss target (points) = a per-campaign configurable ratio (default 50 %) × declared power. This points scale is distinct from both real in-game power and the main goal system's death-points scale (`DEAD_POINTS_PER_T5 = 200` in `kvkGoals.js`) — the three must never be displayed or compared without explicit labelling, mirroring BR-010's domain-separation principle. The main goal formula (F-014) is unchanged. Proposed by the King 2026-07-27; see `Spec_Multicomptes_MainFiller.md` §8. |
