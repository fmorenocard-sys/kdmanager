...
| 4 | ✅ **Page P-008 (F-019)** — **fait le 2026-07-21** | `/kvk-race` + entrée sidebar leadership-only (AccessGate sinon) : duel hero (cartes A/B + écart/variation), classement des 4 camps, tableau royaumes trié DKP (pins 2997/1523 en tête, couverture, cartes mobile), courbes d'évolution du duel (Recharts), sélecteurs campagne/scan — charte v2, i18n ×9 (`nav.kvk_race` ×15 + 15 clés page). | Vérifié en dev (mock fixtures, retiré) : duel +84.3B/+35.1B, pins en tête, graphe rendu, mobile sans scroll horizontal ; invité → AccessGate + nav masquée. Reste V2 : tri par colonne, bouton upload de scan (US-015 UI), vue Joueurs (F-020 Phase 2). |
