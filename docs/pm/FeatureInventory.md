...
| F-018 | KvK Race — Ingestion des scans | Upload des scans de compétition (32 royaumes, ~20 Mo) via URL signée (BR-014) vers le bucket privé + Function `digestRaceScan` (nets base-scan, exclusions, agrégats camps/royaumes, tops joueurs, pré-agrégés Firestore `kvk_race/{cid}`). Portage du moteur Python validé **à parité totale** (jalon 1) et **bout-en-bout cloud vert** (jalon 2, rejeu des 6 scans SoC 4). Reste : formulaire de config King (US-016, jalon 3). | Partiellement Live (jalons 1–2 faits, 2026-07-21) | P1 | Fort | Storage, Plan_Execution_E005_Phase1 |
| F-019 | KvK Race — Dashboard de course | Page P-008 `/kvk-race` (réservée King/Officer, §9.4) : duel hero (DKP net + écart + variation), classement des 4 camps, tableau des royaumes trié DKP avec **pins 2997/1523** et couverture (cartes en mobile), courbes d'évolution du duel multi-scans, sélecteurs campagne/scan. DKP de course paramétrable (BR-010). *V2 : tri par colonne, vue Joueurs/Efficacité (F-020).* | Live (2026-07-21 — Phase 1 complète) | P1 | Fort | F-018 |
